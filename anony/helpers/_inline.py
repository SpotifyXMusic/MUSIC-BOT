...
class Inline:
    ...
    def controls(
        ...
        if not remove:
            keyboard.append(
                [
                    self.ikb(text="Play",   callback_data=f"controls resume {chat_id}"),
                    self.ikb(text="Pause",  callback_data=f"controls pause {chat_id}"),
                    self.ikb(text="Again",  callback_data=f"controls replay {chat_id}"),
    ...
    def lang_markup(self, _lang: str) -> types.InlineKeyboardMarkup:
        ...
        buttons = [
            self.ikb(
                text=f"{name} ({code}) {'[Active]' if code == _lang else ''}",
    ...
    def start_key(
        ...
        # Build owner URL - username works for everyone
        if config.OWNER_USERNAME:
            uname = config.OWNER_USERNAME.lstrip("@")
            owner_url = f"https://t.me/{uname}"
        elif config.OWNER_ID:
            owner_url = f"tg://user?id={config.OWNER_ID}"
        else:
            owner_url = config.SUPPORT_CHAT
